...
function handleErrorResponse(request: NextRequest, resultCode: string, errorMsg: string, status: number) {
  ...
  console.log(isHtml, "KRIS", resultCode, "ddd")
  if (isHtml && resultCode) {
    return NextResponse.redirect(
      new URL(`/results/${encodeURIComponent(resultCode)}?certError=${encodeURIComponent(errorMsg)}`, request.url),
      303
    );
  }
...
export async function GET(
  ...
  try {
    ...
    if (!resultCode) {
      return handleErrorResponse(request, "400", "Result code is required", 400);
    }
    ...
    if (!attempt) {
      return handleErrorResponse(request, resultCode, "Result not found", 404);
    }
    ...
    if (!quiz) {
      return handleErrorResponse(request, resultCode, "Quiz not found", 404);
    }
    ...
    if (attempt.status !== "graded" || !quiz.certificateEnabled) {
      return handleErrorResponse(
        request,
        resultCode,
        "Certificate is not available for this assessment attempt.",
        403
      );
    }
    ...
    if (percentage < minScoreThreshold) {
      return handleErrorResponse(
        request,
        resultCode,
        `Minimum passing threshold of ${minScoreThreshold}% was not reached (scored ${percentage}%).`,
        403
      );
    }
    ...
    return handleErrorResponse(request, "500", errorMessage, 500);
  }
}
