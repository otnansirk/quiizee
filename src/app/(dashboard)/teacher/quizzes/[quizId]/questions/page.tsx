...
export default function QuizQuestionsPage() {
  ...
  const handleAIGenerate = async () => {
    ...
    try {
      ...
      setAISelectedIndexes(new Set(data.questions.map((_: any, i: number) => i)));
      setAIStep('review');
    } catch {
  ...
  const handleAISaveSelected = async () => {
    ...
    if (failed > 0) showToast(`${failed} question${failed > 1 ? 's' : ''} failed to save.`, 'error');
    if (saved > 0) {
    ...
    <div className="min-h-screen animate-fade-in" style={{ paddingBottom: '6rem' }}>
        ...
        <header style={{ marginBottom: '2.5rem' }}>
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.85rem' }}>
              ...
              <button
                onClick={() => { setIsAIModalOpen(true); setAIStep('input'); setAIError(''); }}
                className="btn"
                style={{
      ...
      {isAIModalOpen && (
        <div
          ...
          onClick={(e) => { if (e.target === e.currentTarget) { setIsAIModalOpen(false); setAIStep('input'); } }}
        >
          <div
            className="modal-content card custom-scrollbar"
            style={{
          ...
          >
            ...
            >
              {(['input', 'review'] as const).map((step, i) => (
                <React.Fragment key={step}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <div
                      style={{
                        ...
                        background: aiStep === step ? 'var(--accent-gradient)' : 'rgba(255,255,255,0.07)',
                        color: aiStep === step ? '#fff' : 'var(--text-muted)',
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        fontSize: '0.75rem', fontWeight: 800,
                        boxShadow: aiStep === step ? '0 0 12px rgba(99,102,241,0.4)' : 'none',
                      }}
                    >{i + 1}</div>
                    <span
                      style={{
                        fontSize: '0.8rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em',
                        color: aiStep === step ? 'var(--text-primary)' : 'var(--text-muted)',
                      }}
                    >
                  ...
                  {i === 0 && (
                    <span style={{ color: 'var(--text-muted)', fontSize: '1rem', margin: '0 0.25rem' }}>→</span>
                  )}
                </React.Fragment>
              ))}
            </div>
            ...
            {aiStep === 'input' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                ...
                <div className="form-group" style={{ marginBottom: 0 }}>
                  <label className="label">
                    REFERENCE TEXT / KNOWLEDGE BASE <span style={{ color: 'var(--error)' }}>*</span>
                  </label>
                  <textarea
                    ...
                    rows={7}
                    className="input custom-scrollbar"
                    style={{ resize: 'vertical', minHeight: '140px', fontSize: '0.95rem', lineHeight: 1.6 }}
                  />
                  <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
                  <div className="form-group" style={{ marginBottom: 0 }}>
                    <label className="label">
                      NUMBER OF QUESTIONS: <span style={{ color: 'var(--text-primary)', fontWeight: 900 }}>{aiQuestionCount}</span>
                    </label>
                    <input
                      type="range"
                      min={1}
                      ...
                      className="input"
                      style={{ padding: '0.4rem 0', cursor: 'pointer', accentColor: 'var(--accent)' }}
                    />
                    ...
                    >
                      {(['easy', 'medium', 'hard'] as const).map((d) => (
                        <button
                          ...
                          className="btn"
                          style={{
                            padding: '0.5rem 0.25rem', fontSize: '0.82rem', textTransform: 'capitalize',
                            ...
                            boxShadow: aiDifficulty === d ? '0 4px 12px rgba(99,102,241,0.3)' : 'none',
                          }}
                        >{d}</button>
                      ))}
                    </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.75rem' }}>
                  <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', margin: 0 }}>
                    <span style={{ color: 'var(--text-primary)', fontWeight: 800 }}>{aiSelectedIndexes.size}</span>
                    {' '}of {aiGeneratedQuestions.length} questions selected
                  </p>
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <button
                      type="button"
                      onClick={() => setAISelectedIndexes(new Set(aiGeneratedQuestions.map((_, i) => i)))}
                      className="btn btn-ghost btn-sm"
                    >Select All</button>
                    <button
                      type="button"
                    ...
                    <button
                      type="button"
                      onClick={() => { setAIStep('input'); setAIGeneratedQuestions([]); setAISelectedIndexes(new Set()); }}
                      className="btn btn-secondary btn-sm"
                    >← Re-generate</button>
                  </div>
                </div>
                ...
                <div className="custom-scrollbar" style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem', maxHeight: '400px', overflowY: 'auto', paddingRight: '0.35rem' }}>
                  {aiGeneratedQuestions.map((q, i) => {
                    const selected = aiSelectedIndexes.has(i);
                    const typeLabel = q.type === 'multiple_choice' ? 'Multiple Choice' : q.type === 'true_false' ? 'True / False' : 'Essay';
                    const typeBg =
                      q.type === 'multiple_choice' ? 'rgba(99,102,241,0.15)' :
                      q.type === 'true_false'       ? 'rgba(16,185,129,0.15)' :
                                                      'rgba(245,158,11,0.15)';
                    const typeColor =
                      q.type === 'multiple_choice' ? 'var(--accent-hover)' :
                    ...
                    return (
                      ...
                      >
                        ...
                        <div
                          style={{
                            width: '20px', height: '20px', borderRadius: '5px', flexShrink: 0, marginTop: '3px',
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.4rem', flexWrap: 'wrap' }}>
                            <span
                              className="badge"
                              style={{ margin: 0, fontSize: '0.7rem', background: typeBg, color: typeColor, border: `1px solid ${typeColor}40` }}
                            >{typeLabel}</span>
                            <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)', fontWeight: 600 }}>
                              Q{i + 1} · {q.points} pt{q.points !== 1 ? 's' : ''}
                            </span>
                          </div>
                          <p style={{ color: 'var(--text-primary)', fontSize: '0.9rem', fontWeight: 600, margin: 0, lineHeight: 1.55 }}>
                            {q.questionText}
                          </p>
                          {/* MC options preview */}
                            ...
                            <div style={{ marginTop: '0.5rem', display: 'flex', flexDirection: 'column', gap: '0.2rem' }}>
                              {q.options.map((opt, oi) => (
                                <div
                                  ...
                                  style={{
                                    fontSize: '0.8rem',
                                    color: opt.isCorrect ? '#34d399' : 'var(--text-muted)',
                                    display: 'flex', alignItems: 'center', gap: '0.4rem',
                                  }}
                                >
                                  <span style={{ fontWeight: opt.isCorrect ? 700 : 400 }}>{opt.isCorrect ? '✓' : '·'}</span>
                          ...
                          {q.type === 'true_false' && q.correctAnswer && (
                            <div style={{ marginTop: '0.4rem', fontSize: '0.8rem', color: '#34d399', fontWeight: 600 }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    fontSize: '1rem',
                    fontWeight: 800,
                    background: aiSelectedIndexes.size === 0
                      ? 'rgba(255,255,255,0.05)'
                      : 'linear-gradient(135deg, #10b981 0%, #059669 100%)',
                    color: aiSelectedIndexes.size === 0 ? 'var(--text-muted)' : '#fff',
                    boxShadow: aiSelectedIndexes.size > 0 ? '0 4px 15px rgba(16,185,129,0.35)' : 'none',
                    cursor: isSavingAI || aiSelectedIndexes.size === 0 ? 'not-allowed' : 'pointer',
                ...
                >
                  {isSavingAI
                    ? 'Saving to Quiz...'
                    : `Add ${aiSelectedIndexes.size} Question${aiSelectedIndexes.size !== 1 ? 's' : ''} to Quiz`}
                </button>
              </div>
            )}
          </div>
